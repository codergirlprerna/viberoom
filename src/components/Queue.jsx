      ...
      <div className="absolute inset-0 bg-[#080a08] z-10 flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[.07]">
          ...
          <div className="flex items-center gap-2">
            {/* Add song button always visible on mobile for host */}
            {isHost && (
              <button onClick={onAddSong}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer border border-[rgba(184,247,36,0.3)] text-[#b8f724] bg-[rgba(184,247,36,0.06)] hover:bg-[rgba(184,247,36,0.12)] transition-all">
                + Add Song
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto custom-scroll p-4 flex flex-col gap-2">
          {queue.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center py-10">
              ...
              <div className="text-xs text-[#c8d8c8] mb-4">Add a song to get started</div>
              {isHost && (
                <button onClick={onAddSong}
                  className="px-4 py-2 rounded-xl text-sm font-semibold cursor-pointer border border-[rgba(184,247,36,0.3)] text-[#b8f724] bg-[rgba(184,247,36,0.06)] hover:bg-[rgba(184,247,36,0.12)] transition-all">
                  + Add a song
                </button>
              )}
            </div>
          ...
          {/* Add song at bottom — desktop only (header has it for mobile) */}
          {isHost && queue.length > 0 && (
            <button onClick={onAddSong}
              className="hidden sm:flex items-center gap-3 p-3 rounded-xl border border-dashed border-white/[.1] text-[#c8d8c8] hover:border-[rgba(184,247,36,0.3)] hover:text-[#b8f724] transition-all cursor-pointer bg-transparent w-full mt-1">
